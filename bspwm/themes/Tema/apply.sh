...
## Theme ------------------------------------
#define una variable llamada "BDIR" y la establece como el directorio de configuración de bspwm en el directorio de inicio del usuario.
BDIR="$HOME/.config/bspwm"
#define una variable llamada "TDIR" y la establece como el directorio actual del script mediante el uso de la orden "cd" y el comando "pwd".
TDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )"
#define una variable llamada "THEME" y la establece como el último componente del nombre del directorio de TDIR.
THEME="${TDIR##*/}"

#carga el "theme.bash" 
source "$BDIR"/themes/"$THEME"/theme.bash
#define "altbackground" y "altforeground" y establece su valor como el color de fondo ligeramente modificado mediante el uso de comandos "pastel" para modificar el valor de luz.
...
# 	Este comando específico utiliza sed para realizar una operación de sustitución en el archivo ubicado en la ruta ${PATH_BSPWM}/themes/polybar.sh.

# La opción -i se utiliza para editar el archivo en su lugar, lo que significa que los cambios se guardarán en el mismo archivo. La opción -e se utiliza para especificar que el argumento siguiente es una expresión que debe ser ejecutada.

# La expresión en este comando es "s/STYLE=.*/STYLE=\"$THEME\"/g" y realiza lo siguiente:

# Busca la cadena "STYLE=" seguida de cualquier número de caracteres (indicado por el .*)
# Reemplaza la coincidencia completa con la cadena "STYLE=" seguida del valor de la variable $THEME y una comilla doble de cierre.
# El g al final de la expresión le dice a sed que realice este reemplazo globalmente, es decir, reemplaza todas las coincidencias en el archivo.
...
apply_rofi() {
	...
	# deontro del config.rasi hay atajos
